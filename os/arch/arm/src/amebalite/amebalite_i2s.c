...
#include "amebalite_i2s.h"

#include "mbed/targets/hal/rtl8720e/PinNames.h"
...
#ifndef CONFIG_AMEBALITE_I2S_MAXINFLIGHT
#define CONFIG_AMEBALITE_I2S_MAXINFLIGHT 32
#endif
...
#if defined(CONFIG_AMEBALITE_I2S)

/* Check for I2S RX support */
#if defined(CONFIG_AMEBALITE_I2S_RX)
#define I2S_HAVE_RX     1
#endif

/* Check for I2S TX support */
#if defined(CONFIG_AMEBALITE_I2S_TX)
#define I2S_HAVE_TX     1
#endif

#ifndef CONFIG_AMEBALITE_I2S_DATALEN
#define CONFIG_AMEBALITE_I2S_DATALEN        (I2S_BITS_PER_SAMPLE_16BIT)
#endif

#ifndef CONFIG_AMEBALITE_I2S_SAMPLERATE
#define CONFIG_AMEBALITE_I2S_SAMPLERATE     (SR_44p1KHZ)
#endif
...
struct amebalite_buffer_s {
	struct amebalite_buffer_s *flink;	/* Supports a singly linked list */
...
struct amebalite_i2s_s {
	...
	struct amebalite_transport_s rx;		/* RX transport state */
#endif

	sem_t bufsem_tx;			/* Buffer wait semaphore */
	struct amebalite_buffer_s *freelist_tx;	/* A list a free buffer containers */
	struct amebalite_buffer_s containers_tx[I2S_DMA_PAGE_NUM];

#if defined(I2S_HAVE_TX) && (0 < I2S_HAVE_TX)
	struct amebalite_transport_s tx;		/* TX primary transport state */
#endif

	sem_t bufsem_rx;			/* Buffer wait semaphore */
	struct amebalite_buffer_s *freelist_rx;	/* A list a free buffer containers */
	struct amebalite_buffer_s containers_rx[I2S_DMA_PAGE_NUM];
...
/* Register helpers */
#define CONFIG_AMEBALITE_I2S_DUMPBUFFERS   0

#if defined(CONFIG_AMEBALITE_I2S_DUMPBUFFERS) && (0 < CONFIG_AMEBALITE_I2S_DUMPBUFFERS)
...
/* Semaphore helpers */
static void i2s_exclsem_take(struct amebalite_i2s_s *priv);
...
static int i2s_rxdma_prep(struct amebalite_i2s_s *priv, struct amebalite_buffer_s *bfcontainer);
static int i2s_rx_start(struct amebalite_i2s_s *priv);
static void i2s_rx_worker(void *arg);
static void i2s_rx_schedule(struct amebalite_i2s_s *priv, int result);

static void i2s_bufsem_rx_take(struct amebalite_i2s_s *priv);
#define i2s_bufsem_rx_give(priv) sem_post(&priv->bufsem_rx)

static struct amebalite_buffer_s *i2s_buf_rx_allocate(struct amebalite_i2s_s *priv);
static void i2s_buf_rx_free(struct amebalite_i2s_s *priv, struct amebalite_buffer_s *bfcontainer);
static void i2s_buf_rx_initialize(struct amebalite_i2s_s *priv);
...
static int i2s_txdma_prep(struct amebalite_i2s_s *priv, struct amebalite_buffer_s *bfcontainer);
static int i2s_tx_start(struct amebalite_i2s_s *priv);
static void i2s_tx_worker(void *arg);
static void i2s_tx_schedule(struct amebalite_i2s_s *priv, int result);

static void i2s_bufsem_tx_take(struct amebalite_i2s_s *priv);
#define i2s_bufsem_tx_give(priv) sem_post(&priv->bufsem_tx)

static struct amebalite_buffer_s *i2s_buf_tx_allocate(struct amebalite_i2s_s *priv);
static void i2s_buf_tx_free(struct amebalite_i2s_s *priv, struct amebalite_buffer_s *bfcontainer);
static void i2s_buf_tx_initialize(struct amebalite_i2s_s *priv);
...
static const i2s_config_t i2s_default_config = {
	...
	.sample_rate = CONFIG_AMEBALITE_I2S_SAMPLERATE,
	.bits_per_sample = CONFIG_AMEBALITE_I2S_DATALEN,
...
static struct amebalite_i2s_s *g_i2sdevice[I2S_NUM_MAX] = {NULL};
 ...
 *
 * Input Parameters:
 *   priv - amebalite private i2s data
...
static int i2s_tx_start(struct amebalite_i2s_s *priv)
{
	struct amebalite_buffer_s *bfcontainer = NULL;
	int ret;
	irqstate_t flags;
	int* ptx_buf;
	int pg_idx;
	...
	bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->tx.pend);
...
static void i2s_tx_worker(void *arg)
{
	...
	while (sq_peek(&priv->tx.done) != NULL) {
		...
		flags = irqsave();
		bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->tx.done);
		irqrestore(flags);
...
static void i2s_tx_schedule(struct amebalite_i2s_s *priv, int result)
{
	 ...
	 * both indirectly via the amebalite_dmastop() logic and directly via the
	...
	if (!sq_empty(&priv->tx.act)) {
		...
		bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->tx.act);
...
static uint32_t i2s_txdatawidth(struct i2s_dev_s *dev, int bits)
{
	...
	/* amebalite 16, 24, 32, bits setting */
...
{
	struct amebalite_i2s_s *priv = (struct amebalite_i2s_s *)data;
 ...
 *
 * Input Parameters:
 *   priv - amebalite i2s private data
 *   result - The result of the DMA transfer
 *
 * Returned Value:
 *   None
...
static int i2s_rx_start(struct amebalite_i2s_s *priv)
{
	...
	bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->rx.pend);
...
static void i2s_rx_worker(void *arg)
{
	...
	while (sq_peek(&priv->rx.done) != NULL) {
		...
		flags = irqsave();
		bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->rx.done);
		irqrestore(flags);
...
static void i2s_rx_schedule(struct amebalite_i2s_s *priv, int result)
{
	...
	if (!sq_empty(&priv->rx.act)) {
		...
		bfcontainer = (struct amebalite_buffer_s *)sq_remfirst(&priv->rx.act);
...
/****************************************************************************
 * Name: amebalite_i2s_isr_initialize
 *
 * Description:
 ...
 *
 ****************************************************************************/
int amebalite_i2s_isr_initialize(struct amebalite_i2s_s *priv)
{
...
static void amebalite_i2s_initpins(struct amebalite_i2s_s *priv)
{
...
static void i2s_getdefaultconfig(struct amebalite_i2s_s *priv)
{
...
static int i2s_allocate_wd(struct amebalite_i2s_s *priv)
{
#if defined(I2S_HAVE_RX) && (0 < I2S_HAVE_RX)
	if (priv->rxenab) {
 ...
 *
 ****************************************************************************/
struct i2s_dev_s *amebalite_i2s_initialize(uint16_t port)
{
	if (port >= I2S_NUM_MAX) {
	 ...
	 */
	priv = (struct amebalite_i2s_s *)kmm_zalloc(sizeof(struct amebalite_i2s_s));
	if (!priv) {
	...
	ret = amebalite_i2s_isr_initialize(priv);
	if (ret != OK) {
