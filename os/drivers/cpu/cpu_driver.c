...
static ssize_t cpu_write(FAR struct file *filep, FAR const char *buffer, size_t buflen);
...
static int cpu_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
	...
	cpu = (int)(intptr_t)inode->i_private;
	...
	case IOC_CPU_DISABLE:
		 ...
		 * sched_cpuoff() internally handles:
		 * - Clearing CPU from g_active_cpus_mask
		 * - Task migration via sched_migrate_tasks()
		 * - Calling up_cpu_off() for hardware power-off
		 */
